...
const Game = function (io, name, blind) {
  ...
  this.startNewRound = () => {
    ...
    setTimeout(x => {this.timeOutPlayer(this.roundData.turn)}, 10000);
    this.roundNum++;
    this.rerender();
  };

  this.timeOutPlayer = (playerUsername) => {
    if(this.roundData.turn === playerUsername) {
      console.log('Player is timeout');
      this.moveOntoNextPlayer();
    }
  }

  this.rerender = () => {
    let playersData = this.generatePlayersData();
  ...
  this.getFullGameData = () => {
    ...
    const playersData = this.generatePlayersData();
    return { gameName: this.gameName, gameData, playersData }
  }
    ...
    for(let i = 1; i <= this.maxPlayer; i++) {
      ...
      if(findPlayer) {
        const playerData = {
          ...
          status:findPlayer.getStatus(),
          blind: findPlayer.getBlind(),
          money: findPlayer.getMoney(),
          buyIns: findPlayer.buyIns,
          seat: findPlayer.getSeat(),
          isChecked: this.playerIsChecked(findPlayer),
        }
  ...
  this.getPlayersArray = () => {
    return this.players.map((p) => {
      return { playerName: p.getUsername(), playerSeat: p.getSeat() }
    });
  };
  ...
  this.addPlayer = (playerName, socket, seat) => {
    if (seat) {
      const isAvailable = this.checkSeat(seat);
      if (!isAvailable) {
        seat = this.getSeat();
      }
    } else {
    ...
    const player = new Player(playerName, socket, this.debug, seat);
    this.players.push(player);
    this.updatePlayersPublish();
    this.gameStarter();
    return player;
  };
  ...
  this.updatePlayersPublish = () => {
    io.to(this.gameName).emit('playerUpdate', {
      gameName: this.gameName,
      playersData: this.generatePlayersData(),
    });
  }


  this.updateRoomPublish = () => {
    io.to(this.gameName).emit('RoomUpdate', {
      gameName: this.gameName,
  ...
  this.getAvailableSeats = () => {
    ...
    const noAvailable = [];
    this.players.map(x => noAvailable.push(x.seat));
    for (let i = 1; i <= this.maxPlayer; i++) {
      availableSeats.push(i);
    }
    return _.difference(availableSeats, noAvailable)
  }
  ...
  this.checkSeat = (seatNo) => {
    let available = true;
    this.players.map(x => { if (x.seat === seatNo) { available = false; } });
    return available;
  }
  ...
  this.startGame = () => {
    ...
    this.io.to(this.gameName).emit('gameStart', { gameData: this.getGameData(), playerData: this.generatePlayersData(), players: this.getPlayersSeat()});
    this.startNewRound();
    } else {
      console.log('Dont Have Enought Players To Start Game');
    }
   
  };

  this.getPlayersSeat = () => {
    return this.players.map(x => { return { seat: x.seat, username: x.username  } }) ;
  }
  ...
  this.emitTable = (eventName, payload) => {
  ...
  this.gameStarter = (admin) => {
    if(this.gameStatus === 'On-Wait') {
      if(this.getNumPlayers() > 1) {
        ...
        console.log('Not Have Enought Player To Start!')
      }
    }
